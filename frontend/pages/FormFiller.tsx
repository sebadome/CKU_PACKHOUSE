            ...
            if (comRow && pComRow) {
                ...
                for (let i = 1; i <= 30; i++) {
                    ...
                    nextTable3B.forEach((r: any) => {
                        // Excluimos las filas de resumen para la suma de defectos por línea
                        if (r.concepto === 'Comercial' || r.concepto === '% Comercial') return;
                        
                        const pVal = parseFloat(r[lk]);
                        if (!isNaN(pVal)) {
                            // Reconstruimos unidades a partir del porcentaje guardado: (P / 100) * C
                            totalUnits += Math.round((pVal * calibreMuestra) / 100);
                        }
                    });

                    if (nextComRow[lk] !== totalUnits) {
                        nextComRow[lk] = totalUnits;
                        changed = true;
                    }
                    const pCom = parseFloat(((totalUnits / calibreMuestra) * 100).toFixed(1));
                    if (nextPComRow[lk] !== pCom) {
                        nextPComRow[lk] = pCom;
                        changed = true;
                    }
                }
        ...
        if (Array.isArray(tableMI)) {
            ...
            if (fruitsRow) {
                ...
                for (let i = 1; i <= 30; i++) {
                    ...
                    if (fruitsRow[colKey] !== colSum) {
                        fruitsRow[colKey] = colSum;
                        changedMI = true;
                    }
                }
                ...
                for (let i = 1; i <= 30; i++) {
                    const val = parseFloat(fruitsRow[`f${i}`]);
                    if (!isNaN(val)) totalFrutosGeneral += val;
                }
                if (submission.data.total_frutos_mercado_interno !== totalFrutosGeneral) {
                    handleDataChange('total_frutos_mercado_interno', totalFrutosGeneral, { markAsDirty: false });
                }
            }
                                    ...
                                    <div className="flex items-center gap-6 px-6 py-2 bg-white border border-gray-200 rounded-lg shadow-sm">
                                        <div className="text-center">
                                            ...
                                            <p className={`text-2xl font-bold ${Math.abs((submission.data.total_frutos_mercado_interno || 0) - 100) < 5 ? 'text-green-600' : 'text-cku-blue'}`}>
                                                {submission.data.total_frutos_mercado_interno || 0}
                                            </p>
                                        </div>
