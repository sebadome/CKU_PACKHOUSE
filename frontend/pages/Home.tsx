...
const getPublishedTemplatesFor = (
  ...
  return templates.filter(
    (t) =>
      t.status === 'Publicada' &&
      (!t.publishedTo ||
        t.publishedTo.length === 0 ||
        t.publishedTo.includes(role))
  );
};
...
const Home: React.FC<HomeProps> = ({ templates, submissions }) => {
  ...
  const searchResults = useMemo(() => {
    ...
    if (role === 'TrabajadorCKU') {
        candidateDrafts = candidateDrafts.filter(s => s.submittedBy === user?.nombre);
    }
    ...
    if (role === 'TrabajadorCKU') {
        candidateRecords = candidateRecords.filter(s => s.submittedBy === user?.nombre);
    }
  ...
  }, [searchTerm, publishedTemplates, submissions, role, user?.nombre]);
