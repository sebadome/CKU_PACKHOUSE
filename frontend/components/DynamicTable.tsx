...
export const DynamicTable: React.FC<DynamicTableProps> = ({
  ...
  const newRow = columns.reduce(
    ...
    {
      _id: uuidv4(),
      _isFixed: false,        // 🔑 CLAVE
      _isReadOnlyRow: false  // 🔑 OPCIONAL pero recomendado
  ...
  const getNavigableColumns = (columns: DynamicTableColumn[]) =>
  columns.filter(
    c =>
      !c.readOnly &&
      !c.calc &&
      !['pressure_button', 'findings_button', 'boolean'].includes(c.type)
  );
...
const handleArrowNavigation = (
  ...
  const rows = data || [];
  const navigableCols = getNavigableColumns(columns);
  const colIndex = navigableCols.findIndex(c => c.key === columnKey);
  ...
  if (nextCol < 0 || nextCol >= navigableCols.length) return;

  const nextRowId = rows[nextRow]._id;
  const nextColKey = navigableCols[nextCol].key;
  const nextCellKey = `${nextRowId}-${nextColKey}`;
...
////
  ...
  const renderCellInput = (
    ...
    switch (column.type) {
      ...
      case 'decimal':
        return (
          <input
            ...
            autoComplete="off"
            onKeyDown={(e) =>
              handleArrowNavigation(e, data.findIndex(r => r._id === rowId), column.key)
            }
          />  
        );
      ...
      case 'text':
      default:
        return <input type="text" {...commonProps} value={value ?? ''} key={cellKey} autoComplete="off" className={baseInputClasses} onKeyDown={(e) =>
        handleArrowNavigation(
          e,
          data.findIndex(r => r._id === rowId),
          column.key
            )
          } />;
        }
