...
const Header: React.FC = () => {
  ...
  const getInitials = (nombre: string) => {
    ...
    if (nombreParts.length > 1) {
        const firstInitial = nombreParts[0][0];
        const lastInitial = nombreParts[nombreParts.length - 1][0];
        return `${firstInitial}${lastInitial}`.toUpperCase();
    }
    if (nombreParts.length === 1 && nombreParts[0].length > 0) {
        return nombreParts[0].substring(0, 2).toUpperCase();
    }
        ...
        <div className="flex items-center justify-between h-16 md:h-20 gap-4">
          ...
          <div className="flex items-center space-x-2 sm:space-x-4 overflow-x-auto no-scrollbar py-2 flex-shrink min-w-0">
            <Select 
              ...
              disabled
              aria-label="Planta"
              className="w-28 md:w-36 lg:w-48 text-xs sm:text-sm flex-shrink-0 bg-gray-100 cursor-not-allowed"
            >
              <option value={planta}>
                Planta: {planta
                  .split('_')
